...
namespace log4netContrib.Appender
{
    ...
    public class FallbackAppender : ForwardingAppender
    {
        ...
        {
            LogLog.Error(appender.GetType(),"appender [" + appender.Appender.Name + "] has an error so is not being appended to.");
            if (appenderQueue.Count > 0)
            {
                var nextAppender = appenderQueue.Peek();
                LogLog.Debug(appender.GetType(),"Chaining through to appender [" + nextAppender.Appender.Name + "]");
            }
            else
                LogLog.Error(appender.GetType(),"No more appenders exist to chain through to");
        }
